...
/**
 * Login-Seiten-Komponente, die die Benutzerauthentifizierung verarbeitet
 */
const Login = () => {
  const navigate = useNavigate()
  const { login, error } = useAuth()

  // Zustand für Formulardaten und UI
  ...
  // Effekt zur Überwachung von Login-Versuchen
  useEffect(() => {
    if (attempts >= 3) {
  ...
  const handleSubmit = async (e) => {
    ...
    // Stoppen, wenn Validierungsfehler vorliegen
